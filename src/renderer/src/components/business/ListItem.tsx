  ...
  return (
    <div className="w-full px-3 py-2.5 border-b border-border/50 cursor-pointer transition-colors duration-200 hover:bg-accent/50">
      <div className="flex items-center gap-2 mb-1.5">
        ...
        <span className="font-medium text-sm truncate select-text">title</span>
      </div>
      <div className="flex items-center justify-between text-xs text-muted-foreground">
        <div className="flex items-center gap-1.5 px-1.5 py-0.5 rounded-md bg-primary/10">
          <Tag size={10} className="text-primary" />
          <span className="text-primary/80 select-text">JavaScript</span>
        </div>
        <span className="text-muted-foreground/70 select-text">time</span>
      </div>
    </div>
  )
}
